...
arg_sets=(
    "--input ${INPUT_DIR} --det=DUNE --flav=numu --output ${PLOT_DIR}"
    "--input ${INPUT_DIR} --det=DUNE --flav=nue --output ${PLOT_DIR}"
    "--input ${INPUT_DIR} --det=T2K --flav=numu --output ${PLOT_DIR}"
    "--input ${INPUT_DIR} --det=T2K --flav=nue --output ${PLOT_DIR}"
)
...
for i in "${!arg_sets[@]}"; do
    ...
    job_script=wrongsign_ratio_comp_script_${i}.sh
    ...
    echo "shifter --entrypoint python3 make_wrongsign_ratio_comp.py ${args}" >> ${job_script}
