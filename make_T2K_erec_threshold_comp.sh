...
arg_sets=(
    "--input ${INPUT_DIR} --flux=FHC_numu --output ${PLOT_DIR}"
    "--input ${INPUT_DIR} --flux=RHC_numubar --output ${PLOT_DIR}"
)
...
for i in "${!arg_sets[@]}"; do
    ...
    job_script=T2K_erec_threshold_comp_script_${i}.sh
    ...
    echo "shifter --entrypoint python3 make_T2K_erec_threshold_comp.py ${args}" >> ${job_script}
