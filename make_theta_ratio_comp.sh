...
arg_sets=(
    "--input ${INPUT_DIR} --det=T2K --flux=FHC_numu --sample cc0pi --output ${PLOT_DIR}"
    "--input ${INPUT_DIR} --det=T2K --flux=RHC_numubar --sample cc0pi --output ${PLOT_DIR}"
    "--input ${INPUT_DIR} --det=DUNE --flux=FHC_numu --sample ccinc --output ${PLOT_DIR}"
    "--input ${INPUT_DIR} --det=DUNE --flux=RHC_numubar --sample ccinc --output ${PLOT_DIR}"
)
...
for i in "${!arg_sets[@]}"; do
    ...
    job_script=theta_ratio_comp_script_${i}.sh
    ...
    echo "shifter --entrypoint python3 make_theta_ratio_comp.py ${args}" >> ${job_script}
