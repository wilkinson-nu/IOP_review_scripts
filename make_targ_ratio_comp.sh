...
arg_sets=(
    "--input ${INPUT_DIR} --targ1 Ar40 --targ2 C12 --flav 14 --sample ccinc --output ${PLOT_DIR}"
    "--input ${INPUT_DIR} --targ1 Ar40 --targ2 C12 --flav -14 --sample ccinc --output ${PLOT_DIR}"
    "--input ${INPUT_DIR} --targ1 O16 --targ2 C12 --flav 14 --sample cc0pi --output ${PLOT_DIR}"
    "--input ${INPUT_DIR} --targ1 O16 --targ2 C12 --flav -14 --sample cc0pi --output ${PLOT_DIR}"
)
...
for i in "${!arg_sets[@]}"; do
    ...
    job_script=targ_ratio_comp_script_${i}.sh
    ...
    echo "shifter --entrypoint python3 make_targ_ratio_comp.py ${args}" >> ${job_script}
